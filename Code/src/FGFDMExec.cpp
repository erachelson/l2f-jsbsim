...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
void FGFDMExec::SetSimtime(double ts, double tend, double dt)
{
  StartTime=ts;
  State->Setsim_time(ts);
  State->Setdt(dt);
  EndTime = tend;
}
...
bool FGFDMExec::Run(void)
{
  ...
  if (Script == 0 && State->Getsim_time() > EndTime) success = false;
...
void FGFDMExec::SetAtmosphereWindfield(string filename)
{ 
	Atmosphere->SetWindfield(filename);
}
...
void FGFDMExec::SetElevatorCmd(double cmd)
{ 
	FCS->SetDeCmd(cmd);
}
...
/** WARNING!! This Function is not part of the original JSBSim. Was created by Sreekanth.
    ...
    @param value aileron command in percent*/  
void FGFDMExec::SetAileronCmd(double cmd)
{ 
	FCS->SetDaCmd(cmd);
}
...
/** WARNING!! This Function is not part of the original JSBSim. Was created by Sreekanth.
    ...
    @param value rudder command in percent*/  
void FGFDMExec::SetRudderCmd(double cmd)
{ 
	FCS->SetDaCmd(cmd);
}
...
void FGFDMExec::DoTrim(int mode)
{
  ...
  cout<<"********************************************************"<<endl<<endl
  <<"Trimming the aircraft will reset the latitude and longitude of the simulation to (0.0,0.0)"<<endl<<endl
  <<"********************************************************"<<endl<<endl;
